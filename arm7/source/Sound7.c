...
static bool MODSeek(u32 order, u32 row)
{
    ...
    sndMod.breakRow  = 0;
    sndMod.rowPtr    = sndMod.pattern[sndMod.order[sndMod.curOrder]] + sndMod.curRow * 4 * sndMod.channelCount; // 4 bytes/channel/row
...
static void MODHandleUpdateFlags(MOD_UPDATE_VARS *vars)
{
    ...
    else
    {
        ...
        SCHANNEL_CR(vars->chnIdx)    = SOUND_VOL(vars->sndChn->vol) | SOUND_PAN(vars->sndChn->pan)
                                    | (vars->sndChn->loopLength == 0 ? SOUND_ONE_SHOT : SOUND_REPEAT) | SOUND_FORMAT_8BIT | schnEnable;
    }
