...
bool GetModelCollsisionXY(int num,int x,int y,int rot){
	...
	if(num<0 || num>=MD2_Count)return true;
	if(ModelEnable[num]==false)return true;
	//return Models[num].Image[y];
	if((Models[num].Image[Y] & (1<<X))!=0)return false;
	else return true;
}
...
void MakeCollisionMap(int modelnum){
	if(ModelEnable[modelnum]==false)return;
	...
	for (i = 0; i < Models[modelnum].header.num_tris; ++i){
		for (j = 0; j < 3; ++j){	
			...
			if(j==0)vert2num=Models[modelnum].dreiecke[i].vertex[1];
			if(j==1)vert2num=Models[modelnum].dreiecke[i].vertex[2];
			if(j==2)vert2num=Models[modelnum].dreiecke[i].vertex[0];
