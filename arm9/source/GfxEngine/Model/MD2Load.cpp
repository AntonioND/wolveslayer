...
void FreeModels(void){
	...
	for(a=0;a<MD2_Count;a++){
		if(ModelEnable[a]==true){
			for (N = 0; N < Models[a].header.num_frames; ++N)free(Models[a].rahmen[N].verts);
...
void LoadMD2Model(char Filename[],int num,int widthheight,int scale){
  ...
  if ((Models[num].header.ident != 844121161) ||
  ...
  for (i = 0; i < Models[num].header.num_frames; ++i)
    {
      ...
      Models[num].frames[i].verts = (md2_vertex_t *)
	malloc (sizeof (md2_vertex_t) * Models[num].header.num_vertices);
...
for (i = 0; i < Models[num].header.num_frames; ++i)
	Models[num].rahmen[i].verts = (nds_vertex_t *)
		malloc (sizeof (nds_vertex_t) * Models[num].header.num_vertices);
...
for (N = 0; N < Models[num].header.num_frames; ++N){
	for (I = 0; I < Models[num].header.num_vertices; ++I){
		...
		for(g=0;g<4;g++){
			if(g==0){normz=anormtable16[Models[num].frames[N].verts[I].normalIndex][0];normx=anormtable16[Models[num].frames[N].verts[I].normalIndex][1];}
			if(g==1){normz=anormtable16[Models[num].frames[N].verts[I].normalIndex][1];normx=-anormtable16[Models[num].frames[N].verts[I].normalIndex][0];}
			if(g==2){normz=-anormtable16[Models[num].frames[N].verts[I].normalIndex][0];normx=-anormtable16[Models[num].frames[N].verts[I].normalIndex][1];}
			if(g==3){normz=-anormtable16[Models[num].frames[N].verts[I].normalIndex][1];normx=anormtable16[Models[num].frames[N].verts[I].normalIndex][0];}
	
			if(normz<=zero){
				if(g==0)if(normx<zero)Models[num].rahmen[N].verts[I].colorindex|=0;
				if(g==0)if(normx>=zero)Models[num].rahmen[N].verts[I].colorindex|=1;
				if(g==1)if(normx<zero)Models[num].rahmen[N].verts[I].colorindex|=(0<<2);
				if(g==1)if(normx>=zero)Models[num].rahmen[N].verts[I].colorindex|=(1<<2);
				if(g==2)if(normx<zero)Models[num].rahmen[N].verts[I].colorindex|=(0<<4);
				if(g==2)if(normx>=zero)Models[num].rahmen[N].verts[I].colorindex|=(1<<4);
				if(g==3)if(normx<zero)Models[num].rahmen[N].verts[I].colorindex|=(0<<6);
				if(g==3)if(normx>=zero)Models[num].rahmen[N].verts[I].colorindex|=(1<<6);			
			}
				
			if(normz>zero){
				if(g==0)if(normx<zero)Models[num].rahmen[N].verts[I].colorindex=2;
				if(g==0)if(normx>=zero)Models[num].rahmen[N].verts[I].colorindex=3;
				if(g==1)if(normx<zero)Models[num].rahmen[N].verts[I].colorindex|=(2<<2);
				if(g==1)if(normx>=zero)Models[num].rahmen[N].verts[I].colorindex|=(3<<2);
				if(g==2)if(normx<zero)Models[num].rahmen[N].verts[I].colorindex|=(2<<4);
				if(g==2)if(normx>=zero)Models[num].rahmen[N].verts[I].colorindex|=(3<<4);
				if(g==3)if(normx<zero)Models[num].rahmen[N].verts[I].colorindex|=(2<<6);
				if(g==3)if(normx>=zero)Models[num].rahmen[N].verts[I].colorindex|=(3<<6);
			}
		}	
	}
}
...
for (N = 0; N < Models[num].header.num_frames; ++N)free(Models[num].frames[N].verts);
