...
void ucase (char character[256],char *New)
{
	...
	return; // TODO: Check that this is only used for filesystem files
...
void UnloadStuff(void){
	...
	int a;
	for(a=0;a<Ground_Count;a++)BodenColorKey[a]=0;
	for(a=0;a<Ground_Count;a++)BodenTransEnable[a]=false;
	for(a=0;a<Ground_Count;a++)BodenBumpEnable[a]=false;
	for(a=0;a<4;a++){AutotileColorKeyMaster[a]=0;AutotileIgnorecolorsNum[a]=-1;}
	for(a=0;a<Object_Count;a++)ObjektColorKey[a]=0;
	for(a=0;a<Object_Count;a++)strcpy(ObjektTyp[a],".");
	for(a=0;a<Object_Count;a++)ObjectRadius[a]=0;
	FreeModels();
	ResetDynamic();
	
	PlPosSX=0;PlPosSY=0;
	CamPosSX=0;CamPosSY=0;
	outside=true;
...
void LoadScrip(char filename[]){
	...
	//print atributes if there are some
		...
		else Print("->No Mapfile specified",5,20);
		...
		else Print("->No Musikfile specified",5,30);

		//environment
		if (map->Attribute("env"))
		{
			if(strncmp ("outside",map->Attribute("env"),7) == 0)outside=true;
			if(strncmp ("inside",map->Attribute("env"),6) == 0)outside=false;
		}
		else Print("->No environment specified",5,40);
...
void LoadMapChangeDoor(char* Com){
	...
	if(Keynum>100 || Keynum<0)Keynum=0;
