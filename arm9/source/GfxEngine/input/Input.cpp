...
void Menu(void){
	...
	if(keysHeld() & KEY_TOUCH){
		...
		if(touch.py>144 && touch.py<144+32){ //Buttonfield
			if(touch.px>29 && touch.px<29+64)MouseOnButt=1;
			if(touch.px>29+130 && touch.px<29+129+64)MouseOnButt=2;
		}
	...
	if(!(keysHeld() & KEY_TOUCH) && Old==10){
		if(EquipedWeapon==CellSelect)EquipedWeapon=-1;
		else EquipedWeapon=CellSelect;
		ItemMode();
		s=1;
	}//Equip/DeEquip Weapon
	
	if(!(keysHeld() & KEY_TOUCH) && Old==1){CellSelect=0;ItemMode();s=1;}
	if(!(keysHeld() & KEY_TOUCH) && Old==2 && screenmode<3){PauseMode();s=1;}
	
	if(!(keysHeld() & KEY_TOUCH) && MouseOnButt!=Old && s==0)ScreenMode();
...
void Interact(void){
	...
	//Lets start interact
			...
			if(PlRichtung==0){d=true;}
			if(PlRichtung==1){d=true;l=true;}
			if(PlRichtung==2){l=true;}
			if(PlRichtung==3){u=true;l=true;}
			if(PlRichtung==4){u=true;}
			if(PlRichtung==5){u=true;r=true;}
			if(PlRichtung==6){r=true;}
			if(PlRichtung==7){d=true;r=true;}
	...
	if(PlStatus==1 && PlFrame==21 && Gameended==false){
			...
			for(NPCnum=0;NPCnum<=GegnerCount;NPCnum++){
				...
				if(Py>NPy-GegnerRadius[NPCnum] && Py<NPy+GegnerRadius[NPCnum] && GegnerStatus[NPCnum]<2){
					if(Px<NPx && Px+GegnerRadius[NPCnum]>NPx)nr=NPCnum;
					if(Px>NPx && Px-GegnerRadius[NPCnum]<NPx)nl=NPCnum;
				}
				if(Px>NPx-GegnerRadius[NPCnum] && Px<NPx+GegnerRadius[NPCnum] && GegnerStatus[NPCnum]<2){
					if(Py<NPy && Py+GegnerRadius[NPCnum]>NPy)nd=NPCnum;
					if(Py>NPy && Py-GegnerRadius[NPCnum]<NPy)nu=NPCnum;
				}
			}
...
/*
	...
	if(keysDown() & KEY_A){
		...
		for(NPCnum=0;NPCnum<=DorfiCount;NPCnum++){
			NPx=DorfiX[NPCnum];NPy=DorfiY[NPCnum];
			if(DorfiSY[NPCnum]>=-.5)NPy++;if(DorfiSY[NPCnum]<=.5)NPy--;
			if(DorfiSX[NPCnum]>=-.5)NPx++;if(DorfiSX[NPCnum]<=.5)NPx--;
			NPx+=DorfiSX[NPCnum]+.5;NPy+=DorfiSY[NPCnum]+.5;
			//Wow strange code....but should work to compare those positions right
			//now lets compace		
			if(Py>NPy-.8 && Py<NPy+.8){
				if(Px<NPx && Px+.8>NPx)nr=NPCnum;
				if(Px>NPx && Px-.8<NPx)nl=NPCnum;
			}
			if(Px>NPx-.8 && Px<NPx+.8){
				if(Py<NPy && Py+.8>NPy)nd=NPCnum;
				if(Py>NPy && Py-.8<NPy)nu=NPCnum;
			}
		}
...
void inputs(void){
	...
	SetWallTrans (false);
	if(IsObjHouse(GetPX(),GetPY()+1) || IsObjHouse(GetPX(),GetPY()+2))SetWallTrans (true);
	if(IsObjWall(GetPX(),GetPY()+1)==true)if(ShapeObj[GetPX()][GetPY()+1]==obwalldoor)SetWallTrans (true);
	if(IsObjWall(GetPX(),GetPY()+2)==true)if(ShapeObj[GetPX()][GetPY()+2]==obwalldoor)SetWallTrans (true);
	if(IsObjBumpWall(GetPX(),GetPY()+1)==true)if(ShapeObj[GetPX()][GetPY()+1]==obwalldoor)SetWallTrans (true);
	if(IsObjBumpWall(GetPX(),GetPY()+2)==true)if(ShapeObj[GetPX()][GetPY()+2]==obwalldoor)SetWallTrans (true);
	...
	if(keysHeld() & KEY_DOWN)pressedkeycount++;
	if(keysHeld() & KEY_UP)pressedkeycount++;
	if(keysHeld() & KEY_LEFT)pressedkeycount++;
	if(keysHeld() & KEY_RIGHT)pressedkeycount++;
	
	if(pressedkeycount==2)CheckcolisionDiagonal(PlPosX+6,PlPosY+8,sx+10,sy+10,5,(bool*)&val);
	else Checkcolision(PlPosX+6,PlPosY+8,sx+10,sy+10,5,(bool*)&val);
	...
	for(NPCnum=0;NPCnum<=GegnerCount;NPCnum++){
		...
		//now lets compace		
		if(dx>dy)if(Py>NPy-GegnerRadius[NPCnum] && Py<NPy+GegnerRadius[NPCnum]  && GegnerStatus[NPCnum]!=3){
			if(Px<NPx && Px+GegnerRadius[NPCnum]>NPx)r=false;
			if(Px>NPx && Px-GegnerRadius[NPCnum]<NPx)l=false;
		}
		if(dy>dx)if(Px>NPx-GegnerRadius[NPCnum] && Px<NPx+GegnerRadius[NPCnum]  && GegnerStatus[NPCnum]!=3){
			if(Py<NPy && Py+GegnerRadius[NPCnum]>NPy)d=false;
			if(Py>NPy && Py-GegnerRadius[NPCnum]<NPy)u=false;
		}
	}
	...
	if(PlStatus!=3){
		...
		if(keysHeld() & KEY_DOWN)PlRichtung=0;
		if(keysHeld() & KEY_LEFT)PlRichtung=2;
		if(keysHeld() & KEY_UP)PlRichtung=4;
		if(keysHeld() & KEY_RIGHT)PlRichtung=6;	
	
		if(keysHeld() & KEY_DOWN)if(keysHeld() & KEY_LEFT)PlRichtung=1;
		if(keysHeld() & KEY_DOWN)if(keysHeld() & KEY_RIGHT)PlRichtung=7;	
		if(keysHeld() & KEY_UP)if(keysHeld() & KEY_LEFT)PlRichtung=3;
		if(keysHeld() & KEY_UP)if(keysHeld() & KEY_RIGHT)PlRichtung=5;	
	...
	//linear
		if(pressedkeycount==1){
			if((keysHeld() & KEY_UP)&&u){
				if(ul==true && ur==true)PlPosSY-=.05;//nothing in our way...go ahead
			...
			if((keysHeld() & KEY_DOWN)&&d){
				if(dl==true && dr==true)PlPosSY+=.05;//nothing in our way...go ahead
			...
			if((keysHeld() & KEY_LEFT)&&l){
				if(dl==true && ul==true)PlPosSX-=.05;//nothing in our way...go ahead
			...
			if((keysHeld() & KEY_RIGHT)&&r){
				if(dr==true && ur==true)PlPosSX+=.05;//nothing in our way...go ahead
	...
	//diagonal
		if(pressedkeycount==2){
			...
			if(PlPosSX>.5){PlPosX+=1;PlPosSX-=1;}
			if(PlPosSX<-.5){PlPosX-=1;PlPosSX+=1;}
			if(PlPosSY>.5){PlPosY+=1;PlPosSY-=1;}
			if(PlPosSY<-.5){PlPosY-=1;PlPosSY+=1;}
			...
			CamPosSY=PlPosSY;
			CamPosSX=PlPosSX;
			if(CamPosSY<=-.5){CamPosY-=1;CamPosSY+=1;}
