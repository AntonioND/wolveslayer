	...
	if (ScriptFile!=NULL) {
		...
		while (!feof(ScriptFile)){
			...
			if(strncmp ("ITEM",UcaseCom,4) == 0){
				...
				if(strncmp ("WEAPON",Tmp,6) == 0)waffe=1;
...
/*
	...
	if(strncmp ("[KEY",UcaseCom,4) == 0)
		{Container=(bool*)Key;strcpy(ContName,"[KEY");}
	
	if(strncmp ("[BOOL",UcaseCom,4) == 0)
		{Container=(bool*)Bool;strcpy(ContName,"[BOOL");}
	...
	//Ok whe have some bool types....Key & Bool for now...		
		...
		Operator[2]='\0';
		if(Operator[1]!='=')Operator[1]='S';//Ahh a setoperator
		...
		if(Index>0 && Index<=100){
			...
			if(strncmp ("!=",Operator,2) == 0){
				if(strncmp ("TRUE]",Value,5) == 0 && Container[Index-1]!=true)return true;
				if(strncmp ("FALSE]",Value,6) == 0 && Container[Index-1]!=false)return true;
				return false;
			}
...
/*
	...
	if(strncmp ("[KEY",UcaseCom,4) == 0)return BoolCheck_Set(UcaseCom);
	if(strncmp ("[BOOL",UcaseCom,5) == 0)return BoolCheck_Set(UcaseCom);
