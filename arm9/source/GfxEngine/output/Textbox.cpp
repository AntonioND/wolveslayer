...
/*
	...
	if(wordnum>0){
		if(num!=-1)npctalk=num;
		//Blue box
		WaitForFreeVblank();
		int i,j;
		for(i = 0; i < 256; i++)
			for(j = 0; j < 96; j++)
				if(textbox_bin[i+(j*256)]!=0)BG_GFX_SUB[i+((j+5)*256)] = textbox_pal[textbox_bin[i+(j*256)]];
...
void TextBoxmodeHandler(void){
	if(CurWord < wordnum && Nj<=80){
		if(CurChar==1){
			...
			if(Ni>236){Ni=(1+strlen(Txt[CurWord]))*8;Nj+=10;}
		}
		
		if(Nj<=50)PrintOUT(Txt[CurWord],11+Ni-((1+strlen(Txt[CurWord]))*8),22+Nj,false,CurChar);
		if(Nj<=50)PrintOUT(Txt[CurWord],11+Ni-((1+strlen(Txt[CurWord]))*8)+1,22+Nj+1,true,CurChar);
