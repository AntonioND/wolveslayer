...
void UpdateBar(void){
	...
	if(PlHP<=50)col=RGB15(0,31,31) | BIT(15);
	if(PlHP<=33)col=RGB15(31,15,15) | BIT(15);
...
void ScreenMode(void){
	...
	//Draw IngameBG
	WaitForFreeVblank();	
	for(i = 0; i < 256*256; i++)BG_GFX_SUB[i] = ((u16*)touch_bin)[i]; 
...
void Print(char* Text,int x,int y){
	...
	for(Counter=0;Counter<=255;Counter++){ 
		...
		for(int xx=Char*sw;xx<=Char*sw+(sw-1);xx++)for(int yy=0;yy<=sh;yy++){ 
				if(Font1[xx+yy*668]==0){
					i=x+xx-Char*sw+Counter*(sw+1);
					j=y+yy;
					if(i<256 && j<192)if(i>-1 && j>-1)BG_GFX_SUB[i+(j*256)]=RGB15(31,31,31) | BIT(15);				
				}//Writes Font in black
				
				if(Font1[xx+yy*668]==1){
					i=x+xx-Char*sw+Counter*(sw+1);
					j=y+yy;
					if(screenmode==0)if(i<256 && j<192)if(i>-1 && j>-1)BG_GFX_SUB[i+(j*256)]=((u16*)touch_bin)[i+(j*256)] | BIT(15);				
					if(screenmode==-1)if(i<256 && j<192)if(i>-1 && j>-1)BG_GFX_SUB[i+(j*256)]=((u16*)scrL_bin)[i+(j*256)] | BIT(15);				
				}//Restores BG
			} 
...
/*
	...
	for(j = 0; j < 32; j++){	
		...
		if(GegnerCount==-1){
			if(EquipedWeapon!=CellSelect)if(strncmp ("WEAPON",GiveItemType(Inventory[CellSelect]),6) == 0)
	...
	for(int a=0;a<10;a++){
		...
		if(Inventory[a]==-1)PrintOUT("<none>",135,21+(a*10),true,strlen("<none>"));
		else PrintOUT(GiveItemName(Inventory[a]),135,21+(a*10),true,strlen(GiveItemName(Inventory[a])));
	}
