...
void RenderHouseM (int mode,f32 x, f32 y, f32 z)
{
	...
	if(!walltrans)	glPolyFmt(POLY_ALPHA(31) | POLY_CULL_NONE | POLY_FORMAT_LIGHT0);
	if(walltrans)return;
	glPushMatrix ();
...
void RenderHouseBorder (int mode,int dirrection ,f32 x, f32 y, f32 z)
{
	...
	if(!walltrans || mode==1)glPolyFmt(POLY_ALPHA(31) | POLY_CULL_NONE | POLY_FORMAT_LIGHT0|POLY_ID(3));
	if(walltrans)glPolyFmt(POLY_ALPHA(7) | POLY_CULL_NONE | POLY_FORMAT_LIGHT0|POLY_ID(3));
	glPushMatrix ();
...
void RenderHouseCorner (int mode,int dirrection ,f32 x, f32 y, f32 z)
{
	...
	if(dirrection==0){
		LightX1=WallX;LightY1=WallY+1;
		LightX2=WallX+1;LightY2=WallY+1;
		LightX3=WallX;LightY3=WallY;
	}
	
	if(dirrection==1){
		LightX1=WallX;LightY1=WallY;
		LightX2=WallX;LightY2=WallY+1;
		LightX3=WallX+1;LightY3=WallY;
	}	
	
	if(dirrection==2){
		LightX1=WallX+1;LightY1=WallY;
		LightX2=WallX;LightY2=WallY;
		LightX3=WallX+1;LightY3=WallY+1;
	}
	
	if(dirrection==3){
		LightX1=WallX+1;LightY1=WallY+1;
		LightX2=WallX+1;LightY2=WallY;
		LightX3=WallX;LightY3=WallY+1;
	}
