...
float GetInterPolY(int x,int y,float sx, float sy){
	if(x<1)x=0;
	if(x>MapGetWr())x=MapGetWr();
	if(y<1)x=0;
	if(y>MapGetHr())y=MapGetHr();
...
void RefreshMap(void){
	...
	for(int yy=11;yy>-2;yy--){
		...
		minx=5;maxx=6;
		if(yy==11 ||yy==10 || yy==9 )				{minx=2; maxx=9;}
		if(yy==8)									{minx=1; maxx=9;}
		if(yy==7 ||yy==6 || yy==5)				{minx=1; maxx=10;}
		if(yy==4 ||yy==3 || yy==2 )				{minx=0; maxx=11;}
		if(yy==1||yy==0||yy==-1)					{minx=-1; maxx=12;}
		if(yy<=-2)									{minx=-2; maxx=13;}
		
		for(int xx=maxx;xx>minx;xx--){
				x=xx+CamPosX;y=yy+CamPosY;
				if(ObjectNoGround[TexObj[x][y]]==false){
					...
					if(x<MapGetWr() && y<MapGetHr() && ShapeObj[x][y]!=Wallbor)if(x>-1 && y>-1){
						if(TexBod[x][y]>=0 && SetABod[x][y]<0){
								if(TexBod[x][y]>=0)RenderBoden ((xx)-6, 0, (yy)-8,TexBod[x][y]);	
						}//Boden
		
						
						if(TexABod[x][y]!=-2 && SetABod[x][y]>=0 && SetABod[x][y]<=3 && TexBod[x][y]==-2  && ShapeObj[x][y]!=Wallbor){
								subset=0;
								if(TexABod[x][y]>15)subset=1;
...
void RefreshWelt(void){
	...
	for(int yy=12;yy>-4;yy--){
		...
		for(int xx=maxx;xx>minx;xx--){
			...
			seeable=true;
			if(x>=MapGetWr() || y>=MapGetHr())seeable=false;
			if(x<0 || y<0)seeable=false;
			...
			if(seeable){
				...
				//Models
					if(ShapeObj[x][y]==model){
						...
						if(yy>-1 || ((Precalcdata[x][y]&(1<<mirrow)) && ObjectMirrow[TexObj[x][y]]==true)){
							if(!ObjectIllumination[TexObj[x][y]])SetObjLights(pos,DirObj[x][y]*64);//way faster and enough for objects
							else SetObjLightsSelfilluminated();
				...
				//House middle
					if(ShapeObj[x][y]==Housem){
						...
						if(yy>-1)RenderHouseM (0,posx, floattof32(1.0+GetHight(x,y)), posy);//Normal
					}
	
				//House border
					if(ShapeObj[x][y]==Housebor){
						SetCurWall(xx+CamPosX,yy+CamPosY);
						if(yy>-1)RenderHouseBorder (0,DirObj[x][y],posx, floattof32(1.0+GetHight(x,y)), posy);//Normal
						if((Precalcdata[x][y]&(1<<mirrow)))RenderHouseBorder (1,DirObj[x][y],posx, floattof32(-.9-GetHight(x,y)), posy);//Gespiegelt
					}
	
				//House corner
					if(ShapeObj[x][y]==Housecor){
						SetCurWall(xx+CamPosX,yy+CamPosY);
						if(yy>-1)RenderHouseCorner (0,DirObj[x][y],posx, floattof32(1.0+GetHight(x,y)), posy);//Normal
						if((Precalcdata[x][y]&(1<<mirrow)))RenderHouseCorner (1,DirObj[x][y],posx, floattof32(-.9-GetHight(x,y)), posy);//Gespiegelt
				...
				//Wall border
					...
					if(ShapeObj[x][y]==Wallbor){
						SetCurWall(xx+CamPosX,yy+CamPosY);
						if(yy>-1)RenderWallBorder (0,DirObj[x][y],posx, floattof32(1.0+GetHight(x,y)), posy,TexObj[x][y]);//Normal
						if((Precalcdata[x][y]&(1<<mirrow)))RenderWallBorder(1,DirObj[x][y],posx, floattof32(-1-GetHight(x,y)), posy,TexObj[x][y]);//Gespiegelt
					}
...
void RefreshPlayer(void){
	...
	int pos[2];pos[0]=GetPX();pos[1]=GetPY();
	float spos[2];spos[0]=sx;spos[1]=sy;
	...
	if(PlStatus==0){
		if(GegnerCount>=0)aniwaffe=22+stackt11;
		if(screenmode<2 &&((keysHeld() & KEY_DOWN)
			||(keysHeld() & KEY_UP)||(keysHeld() & KEY_LEFT)
				||(keysHeld() & KEY_RIGHT)))aniwaffe+=11;
	}
	...
	if(PlStatus==1){
		...
		if((keysHeld() & KEY_DOWN)
			||(keysHeld() & KEY_UP)||(keysHeld() & KEY_LEFT)
				||(keysHeld() & KEY_RIGHT))aniwaffe=144+(PlFrame/3);	
	}

	//being hitten
	if(PlStatus==2)aniwaffe=110+PlFrame;

	//Dieing
	if(PlStatus==3)aniwaffe=132+(PlFrame/2);

	//animation when winning
	if(PlStatus==-1)aniwaffe=122+(PlFrame/2);
...
//increase frame of animations
	...
	if(PlStatus==1){
		PlFrame++;
		if(PlFrame>=33)PlStatus=0;
	}

	//beeing hitten
	if(PlStatus==2){
		PlFrame++;
		if(PlFrame>=11)PlStatus=0;
	}
	
	//dieing
	if(PlStatus==3){
		PlFrame++;
		if(PlFrame>=22)PlFrame=22;
	}
	
	//winning
	if(PlStatus==-1){
		PlFrame++;
		if(PlFrame>=22)PlFrame=0;
	}
}
...
void RefreshEnemys(void){
	...
	for(a=0;a<=GegnerCount;a++){
		...
		aniset=11+stackt11;
		//attacking
		if(GegnerStatus[a]==1)aniset=22+float(GegnerFrame[a]/22.0*18);
		//being hitten
		if(GegnerStatus[a]==2)aniset=33+GegnerFrame[a];
		//dies
		if(GegnerStatus[a]==3){
		...
		if((GegnerY[a]-8)+GegnerSY[a]-(CamPosY)-CamPosSY<3)
		{
			...
			if(GegnerStatus[a]>=2){
				...
				if(GegnerFrame[a]>44 && GegnerStatus[a]==3)GegnerFrame[a]=44;
			}
