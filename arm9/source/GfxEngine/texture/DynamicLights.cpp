	...
	if(screenmode<3){
		for(a=0;a<10;a++){
			if(Fireflys[a].Enable==true){
				...
				for(b=2;b<8;b++){
					if(Passable(Fireflys[a].X,Fireflys[a].Y,sx+b,sy)==false)u=false;
					if(Passable(Fireflys[a].X,Fireflys[a].Y,sx+b,sy+8)==false)d=false;
					if(Passable(Fireflys[a].X,Fireflys[a].Y,sx+1,sy-1+b)==false)l=false;
					if(Passable(Fireflys[a].X,Fireflys[a].Y,sx+8,sy-1+b)==false)r=false;
				}
	
				if(Fireflys[a].X==MapGetWr())r=false;
				if(Fireflys[a].Y==MapGetHr())d=false;
				
						change=false;
						if((Fireflys[a].Richtung==0 || Fireflys[a].Richtung==1 || Fireflys[a].Richtung==7) && d==false)change=true;
						if((Fireflys[a].Richtung==1 || Fireflys[a].Richtung==2 || Fireflys[a].Richtung==3) && l==false)change=true;
						if((Fireflys[a].Richtung==6 || Fireflys[a].Richtung==5 || Fireflys[a].Richtung==7) && r==false)change=true;
						if((Fireflys[a].Richtung==4 || Fireflys[a].Richtung==3 || Fireflys[a].Richtung==5) && u==false)change=true;
						if(rand()%20==0)change=true;
						
						if(change)TurnDyn(a,l,r,u,d);
					...
					//Horizontal
						if((Fireflys[a].Richtung==6 || Fireflys[a].Richtung==5 || Fireflys[a].Richtung==7) && r)Fireflys[a].SX+=1;
							if(Fireflys[a].SX>19){Fireflys[a].X+=1;Fireflys[a].SX=0;}
						if((Fireflys[a].Richtung==1 || Fireflys[a].Richtung==2 || Fireflys[a].Richtung==3) && l)Fireflys[a].SX-=1;
							if(Fireflys[a].SX<0){Fireflys[a].X-=1;Fireflys[a].SX=19;}
					//Vertikal
						if((Fireflys[a].Richtung==0 || Fireflys[a].Richtung==1 || Fireflys[a].Richtung==7) && d)Fireflys[a].SY+=1;
							if(Fireflys[a].SY>19){Fireflys[a].Y+=1;Fireflys[a].SY=0;}
						if((Fireflys[a].Richtung==4 || Fireflys[a].Richtung==3 || Fireflys[a].Richtung==5) && u)Fireflys[a].SY-=1;
							if(Fireflys[a].SY<0){Fireflys[a].Y-=1;Fireflys[a].SY=19;}			
			}
		}
...
void UpdateViewableLights(void){
	...
	for(int yy=13;yy>-4;yy--){
		y=yy+CamPosY;
		minx=4;maxx=7;
		if(yy==13||yy==12 ||yy==11 ||yy==10 || yy==9 )				{minx=1; maxx=10;}
		if(yy==8)									{minx=0; maxx=10;}
		if(yy==7 ||yy==6 || yy==5)				{minx=0; maxx=11;}
		if(yy==4 ||yy==3 || yy==2 )				{minx=-1; maxx=12;}
		if(yy==1||yy==0 ||yy==-1)					{minx=-2; maxx=13;}
		if(yy<=-2)									{minx=-3; maxx=14;}
