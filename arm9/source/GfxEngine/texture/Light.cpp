...
void RunTime(void){
	...
	if(screenmode<2)Time2++;
	if(Time2>2400)Time2=0;
	
	if(outside)Time=Time2;
	if(!outside)Time=0;
	...
	if(Envr<=255)EnvR=Envr;
	if(Envg<=255)EnvG=Envg;
	if(Envb<=255)EnvB=Envb;
	if(Blend!=8){
		EnvR=EnvR/8*Blend;
		EnvG=EnvG/8*Blend;
		EnvB=EnvB/8*Blend;
...
void GiveLight(int x,int y,u8* col){
	...
	//if(TerrainIsStair[x][y]==false){
		if(pointer.v[0]>pointer.v[1])nx=1;
		if(pointer.v[0]<pointer.v[1])nx=-1;
		if(pointer.v[0]>pointer.v[2])ny=1;
		if(pointer.v[0]<pointer.v[2])ny=-1;
	//}
...
void GrapLightInterpolate(int x,int y,float sx, float sy,int* col){
	...
	r1=inttof32(WorldLightR[x+(y*128)]);g1=inttof32(WorldLightG[x+(y*128)]);b1=inttof32(WorldLightB[x+(y*128)]);
	r2=inttof32(WorldLightR[x+1+(y*128)]);g2=inttof32(WorldLightG[x+1+(y*128)]);b2=inttof32(WorldLightB[x+1+(y*128)]);
	r3=inttof32(WorldLightR[x+((y+1)*128)]);g3=inttof32(WorldLightG[x+((y+1)*128)]);b3=inttof32(WorldLightB[x+((y+1)*128)]);
	r4=inttof32(WorldLightR[x+1+((y+1)*128)]);g4=inttof32(WorldLightG[x+1+((y+1)*128)]);b4=inttof32(WorldLightB[x+1+((y+1)*128)]);
	...
	col[0]=(u8)f32toint(r);col[1]=(u8)f32toint(g);col[2]=(u8)f32toint(b); 
}
...
void SetMdlLights(int pos[2],float softpos[2],float radius,int rot){
	if(rot>=0)rotation=0;
	if(rot>=127)rotation=1;
	if(rot>=255)rotation=2;
	if(rot>=383)rotation=3;
	...
	float sposx1=softpos[0]-radius;if(sposx1<0){sposx1++;posx1--;}
	float sposx2=softpos[0]+radius;if(sposx2>1){sposx2--;posx2++;}
	float sposx3=softpos[0]-radius;if(sposx3<0){sposx3++;posx3--;}
	float sposx4=softpos[0]+radius;if(sposx4>1){sposx4--;posx4++;}

	float sposy1=softpos[1]-radius;if(sposy1<0){sposy1++;posy1--;}
	float sposy2=softpos[1]-radius;if(sposy2<0){sposy2++;posy2--;}
	float sposy3=softpos[1]+radius;if(sposy3>1){sposy3--;posy3++;}
	float sposy4=softpos[1]+radius;if(sposy4>1){sposy4--;posy4++;}
	...
	MdlLight[0][0]+=EnvR;MdlLight[0][1]+=EnvG;MdlLight[0][2]+=EnvB;
	MdlLight[1][0]+=EnvR;MdlLight[1][1]+=EnvG;MdlLight[1][2]+=EnvB;
	MdlLight[2][0]+=EnvR;MdlLight[2][1]+=EnvG;MdlLight[2][2]+=EnvB;
	MdlLight[3][0]+=EnvR;MdlLight[3][1]+=EnvG;MdlLight[3][2]+=EnvB;

	for(int a=0;a<3;a++){
		if(MdlLight[0][a]>255)MdlLight[0][a]=255;
		if(MdlLight[1][a]>255)MdlLight[1][a]=255;
		if(MdlLight[2][a]>255)MdlLight[2][a]=255;
		if(MdlLight[3][a]>255)MdlLight[3][a]=255;
	
		if(Blend!=8)MdlLight[0][a]=MdlLight[0][a]/8*Blend;
		if(Blend!=8)MdlLight[1][a]=MdlLight[1][a]/8*Blend;
		if(Blend!=8)MdlLight[2][a]=MdlLight[2][a]/8*Blend;
		if(Blend!=8)MdlLight[3][a]=MdlLight[3][a]/8*Blend;
	}
...
void SetObjLights(int pos[2],int rot){
	...
	extern int CamPosX,CamPosY;
	extern u16 ViewportMapLights[20][20];
	xx=pos[0]-CamPosX;yy=pos[1]-CamPosY;
...
void SetObjLightsSelfilluminated(void){
	...
	if(Blend!=8)zw=(255/8*Blend);
	
	if(Blend==8)white=(u32)RGB15((u8)255>>3, (u8)255>>3, (u8)255>>3);
	else white=(u32)RGB15((u8)zw>>3, (u8)zw>>3, (u8)zw>>3);
